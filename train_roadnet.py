...
model = RoadNet(3, 1, 16)
# model.load_state_dict(torch.load('/home/aimenext/luantt/road_seg/weights/resnet_fcloss_last.pt'))
...
def validate(threshold = 0.5):
    ...
    for _ in range(n_iter):
        for i, data in enumerate(val_loader):
            ...
            outputs = model(images)[0].sigmoid()
...
for epoch in range(epochs):
    ...
    for i, data in enumerate(train_loader):
        ...
        images = data['image'].to(device)
        segments = data['segment'].to(device)
        edges = data['edge'].to(device)
        centers = data['center_line'].to(device)
        segment_out, edge_out, center_out = model(images)
        segment_loss = criterion(segment_out, segments)
        edge_loss = criterion(edge_out, edges)
        center_loss = criterion(center_out, centers)
        loss = 0.7*segment_loss + 0.1* edge_loss + 0.2 * center_loss
        loss.backward()
        optim.step()
